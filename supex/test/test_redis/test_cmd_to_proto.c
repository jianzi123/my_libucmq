/**
 * @brief Unit test for converting redis command to protocol prototype by shishengjie
 */
...
void case_c2p_all()
{
        int ret;
        char *proto; 
        const char *set_key_value_proto = "*3\r\n"
                "$3\r\n"
                "set\r\n"
                "$3\r\n"
                "key\r\n"
        ...
        ret = cmd_to_proto(&proto, "set key value");
        assert(ret == strlen(set_key_value_proto));
        assert(strcmp(proto, set_key_value_proto) == 0);
        free(proto);

        ret = cmd_to_proto(&proto, "set key %s", "value");
        assert(ret == strlen(set_key_value_proto));
        assert(strcmp(proto, set_key_value_proto) == 0);
        free(proto);
        
        ret = cmd_to_proto(&proto, "set %s value", "key");
        assert(ret == strlen(set_key_value_proto));
        assert(strcmp(proto, set_key_value_proto) == 0);
        free(proto);

        ret = cmd_to_proto(&proto, "set %s %s", "key", "value");
        assert(ret == strlen(set_key_value_proto));
        assert(strcmp(proto, set_key_value_proto) == 0);
        free(proto);

        ret = cmd_to_proto(&proto, "invalid_cmd");
        assert(ret > 0);
        free(proto);
        
        ret = cmd_to_proto(&proto, "");
        assert(ret > 0);
        free(proto);
}
