...
local function get_oil_price (lon, lat)
	...
	local ok, code_jo = redis_api.cmd('mapGridOnePercent01', 'hgetall', grid)
...
function handle()
	...
	local ok, array = redis_api.cmd('private', 'smembers', idx_key)
	...
	for _,idx in pairs(array or {}) do
		if carry_key then
			local ok, info = redis_api.cmd('private', 'hget', carry_key, tostring(idx))
		...
		if wb["multimediaURL"] then
			...
			if ok then
				...
				local ok, imei =  redis_api.cmd('private','get', string.format("%s:IMEI", accountID))
				if ok and imei then
					ok, travelID = redis_api.cmd('private','get', string.format("%s:travelID", imei))
				end
				local ok_date, cur_date = redis_api.cmd('private','get', string.format("%s:speedDistribution", accountID))
				if not ok_date or not cur_date then
					cur_date = os.date("%Y%m")
					redis_api.cmd('private','set', string.format("%s:speedDistribution", accountID), cur_date)
				end 
				if ok and imei and accountID and travelID and cur_date then
					...
					redis_api.cmd('dataCoreRedis','sadd', datacore_statistics_var_key, datacore_statistics_var_value)
					redis_api.cmd('dataCoreRedis','expire', datacore_statistics_var_key, 48*3600)
