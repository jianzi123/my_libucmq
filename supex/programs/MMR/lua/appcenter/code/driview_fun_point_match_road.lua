...
module("driview_fun_point_match_road", package.seeall)
...
local function quick_location( lon, lat, dir, accountID, need )
	...
	--local ok,old_roadID = redis_api.cmd("private", accountID, "get", accountID .. ":sysInternalRoadID")
        local ok, old_roadID = libkv.libkv_hash_cache_get('driview',accountID,accountID .. ":sysInternalRoadID")
	...
	if need == 1 then
		...
		if new_roadID ~= old_roadID then
			...
			--redis_api.cmd("private", accountID, "set", key , value)
                        libkv.libkv_hash_cache_set('driview',accountID, key, value)
		end
	end
	return true,result
end
