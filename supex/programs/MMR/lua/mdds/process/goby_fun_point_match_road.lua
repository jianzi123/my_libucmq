...
module("goby_fun_point_match_road", package.seeall)
...
function get_nexus_grid_key( lon, lat, key_format )
...
function point_match_road( lon, lat, dir, list, need )
	-->> parse data
	local array_lineID = {}
	local array_length = {}
	local array_angle1 = {}
...
local function quick_location( lon, lat, dir, accountID, need )
	...
	local ok,old_roadID = redis_api.cmd("private_hash", accountID or '', "get", accountID .. ":sysInternalRoadID")
	...
	if need == 1 then
		...
		if new_roadID ~= old_roadID then
			...
			redis_api.cmd("private_hash", accountID or '', "set", key , value)
		end
	end
	return true,result
end
