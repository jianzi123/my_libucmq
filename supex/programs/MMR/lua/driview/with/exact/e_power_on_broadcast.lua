...
local function get_tab_to_kv_app( )
	...
	if #tab.accountID == 15 then
		...
		local ok_status,ok_val = redis_api.cmd('private',APP_POOL["OUR_BODY_TABLE"]["accountID"],'get',
								tab.accountID .. ":IMEI")
...
function check_solarcalendar_is_send(body_data)
	...
	local ok_status,ok_cur = redis_api.cmd('private',APP_POOL["OUR_BODY_TABLE"]["accountID"],'get',str_key)
	if not ok_status then 
...
function work_to_poweron_standardapp(body_data)
	...
	local ok_status,ok_ret = redis_api.cmd('private',APP_POOL["OUR_BODY_TABLE"]["accountID"], 'get', 
							accountID .. ':newbieGuide')
