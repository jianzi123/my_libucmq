...
--local data_combination  = require("data_combination")
...
function app_task_forward( app_name )
    ...
    --local info_table = data_combination.info_combination( app_name )
	if not ok then
...
function half_url_incr_idx_send_weibo( app_name )
	...
	local ok, idx = redis_api.cmd('driview',APP_POOL["OUR_BODY_TABLE"]["accountID"], 'get', string.format("%s:%s", accountID, idx_key))
	...
	if carry_key then
		local ok, info = redis_api.cmd('driview',APP_POOL["OUR_BODY_TABLE"]["accountID"], 'get', accountID .. ':' .. carry_key)
		if ok and info then
			local ok,jo = pcall(cjson.decode, info)
			if ok and jo then
				for k,v in pairs(jo) do
