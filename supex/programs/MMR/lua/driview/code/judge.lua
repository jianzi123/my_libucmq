...
--local set_info = require("data_combination")
...
function is_continuous_driving_mileage_point(app_name)
	...
	increase  =  increase and tonumber(increase) or 10
        ...
        local ok,avgSpeed = redis_api.cmd('Mileage', APP_POOL["OUR_BODY_TABLE"]["accountID"], 'hget' , hash_table_name, 'avgSpeed')
        if not ok then
                return false
        end
        local ok,stopTime = redis_api.cmd('Mileage', APP_POOL["OUR_BODY_TABLE"]["accountID"], 'hget', hash_table_name, 'stopTime')
        if not ok then
                return false
        end
       ...
       -- set_info.set_data("l_f_continuous_driving_mileage",carry_data)
