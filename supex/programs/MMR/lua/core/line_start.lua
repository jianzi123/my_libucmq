...
local APP_MONITOR = require('monitor')
...
function add_origin_key_value(key, value)
	if  key then
		...
		APP_POOL["OUR_BODY_TABLE"][key] = value
	end
end

function app_call( top, sfd )
	only.log("D", '_________________________________START_________________________________________')
	APP_POOL["_FINAL_STAGE"] = top
	APP_POOL["_SOCKET_HANDLE"] = sfd
	--> get data
	APP_POOL["OUR_BODY_DATA"] = app_lua_get_body_data(sfd)
	only.log("I", "BODY DATA is:%s", APP_POOL["OUR_BODY_DATA"])
	APP_POOL["OUR_BODY_TABLE"] = APP_UTILS.parse_http_body_data( app_lua_get_head_data(sfd), APP_POOL["OUR_BODY_DATA"] ) or {}
	local our_body_table = APP_POOL["OUR_BODY_TABLE"]

	--> get args
	APP_POOL["OUR_URI_ARGS"] = app_lua_get_uri_args(sfd)
	only.log("I", "URI DATA is:%s", APP_POOL["OUR_URI_ARGS"])
	...
	--]]
	if APP_CFG["OWN_INFO"]["CUSTOM_ORIENTED_SYSTEM"] == true then
		if utils.is_in_table( our_body_table["accountID"], APP_CFG["TEST_ACCOUNT"] ) then
			local request_buf = app_lua_get_recv_buf(sfd)
			only.log("I", "----------------custom oriented system:user data[%s]------------", request_buf)
			only.log("I", "----------------------is a testing user-----------------------")
			local path= "publicentry"
			usr_testing_transmit(request_buf,path)
			return
		end
