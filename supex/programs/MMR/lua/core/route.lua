...
local OWN_APP_HOME_PATH = APP_CFG["OWN_INFO"]["DEFAULT_APP_PATH"]
...
local function save_one_list( t, f, c )
	if not APP_POOL["_FINAL_STAGE"] then return end

	only.log("D", string.format("save json data to file:%s", LIB_CJSON.encode( t )))
	local info = APP_SCAN.dump(t)
...
function pull_templet( mode, name )
	...
	if (not mode) or (mode == "all") then
		return string.format('[{"name":"精准模板","value":%s},{"name":"部分模板","value":%s},{"name":"广播模板","value":%s},{"name":"点播模板","value":%s}]',
			LIB_CJSON.encode( OWN_TEMPLET_LIST_INFO["EXACT_NAME"] ),
			LIB_CJSON.encode( OWN_TEMPLET_LIST_INFO["LOCAL_NAME"] ),
			LIB_CJSON.encode( OWN_TEMPLET_LIST_INFO["WHOLE_NAME"] ),
			LIB_CJSON.encode( OWN_TEMPLET_LIST_INFO["ALONE_NAME"] ))
	else
		return LIB_CJSON.encode( OWN_TEMPLET_LIST_INFO[ string.upper(mode) .. "_NAME" ] )
	end
end
--<[===============================STATUS DATA===============================]>--
...
function make_new_app( mode, name, args, func )
	...
	-->> set head
	local head = 'local APP_POOL = require("pool")\n' ..
	'local APP_UTILS = require("utils")\n' ..
	...
	local keys = string.gsub(LIB_CJSON.encode( memb ), '"position"', '"longitude","latitude"')
	...
	local way = {
		...
		["function"] = function(val, key)
			assert(APP_BOOL_FUNC_LIST["OWN_LIST"][ val ], string.format("[%s] is not in BOOL_FUNC_LIST.lua", val))
			return APP_BOOL_FUNC_LIST["OWN_LIST"][ val ]( name, key ) 
		end
	}
	local match = 'function match()\n' .. string.format('\tif not (%s) then\n\t\treturn false \n\tend\n', string.format("APP_JUDGE.user_control('%s')", name))
	...
	APP_UTILS.safe_cntl_sort( keys_list, true, sort_func )
	for _,f in ipairs(keys_list) do
		local all = vals_list[ f ] 
		for _,one in ipairs(all or {}) do
			local t = one[1]
			local k = one[2]
			local key = string.format( 'APP_POOL["OUR_BODY_TABLE"]["%s"]', tostring(k) )
	...
	-->> set work
	local work = string.format('function work()\n\tAPP_ONLY.log("I", "%s working ... ")\n', name)
	for _,v in pairs(func or {}) do
		work = work .. string.format('\tAPP_WORK_FUNC_LIST["%s"]( "%s" )\n', v, name)
	end
	work = work .. 'end\n\n'
...
function pull_status( )
	return string.format('[{"name":"精准模式","value":%s},{"name":"部分模式","value":%s},{"name":"广播模式","value":%s},{"name":"点播模式","value":%s}]',
		LIB_CJSON.encode(OWN_STATUS_LIST_INFO[ OWN_EXACT_MODE ]),
		LIB_CJSON.encode(OWN_STATUS_LIST_INFO[ OWN_LOCAL_MODE ]), 
		LIB_CJSON.encode(OWN_STATUS_LIST_INFO[ OWN_WHOLE_MODE ]),
		LIB_CJSON.encode(OWN_STATUS_LIST_INFO[ OWN_ALONE_MODE ]))
end
--<[===============================CONFIG DATA===============================]>--
...
function make_new_cfg( name, args, func )
	...
	for i=1, #(func or {}) do
		cfgs["work"][ func[i] ] = APP_WORK_FUNC_LIST["OWN_ARGS"][ func[i] ]
	end
	for k,v in pairs(args or {}) do
		if v[1] == "function" then
			cfgs["bool"][ v[2] ] = APP_BOOL_FUNC_LIST["OWN_ARGS"][ v[2] ]
		end
	end
	cfgs["ways"] = APP_WORK_FUNC_LIST["OWN_MUST"]
...
function pull_config( name )
	--return LIB_CJSON.encode( OWN_CONFIG_LIST_INFO[ name ] or {} )
	local info = LIB_CJSON.encode( OWN_CONFIG_LIST_INFO[ name ] or {} )
...
function pull_explain( name )
	...
	if OWN_EXPLAIN_LIST_INFO[ name ] then
		for k,v in pairs(OWN_EXPLAIN_LIST_INFO[ name ]["bool"] or {}) do
			info["bool"][ v ] = APP_BOOL_FUNC_LIST["OWN_HINT"][ v ]
		end
		for k,v in pairs(OWN_EXPLAIN_LIST_INFO[ name ]["work"] or {}) do
			info["work"][ v ] = APP_WORK_FUNC_LIST["OWN_HINT"][ v ]
		end
	end
	info["ways"] = APP_WORK_FUNC_LIST["OWN_JOIN"]
	return LIB_CJSON.encode( info )
end
--<[===============================ALIAS DATA===============================]>--
...
function pull_alias( )
	return LIB_CJSON.encode( OWN_ALIAS_LIST_INFO or {} )
end
...
function clean_old_idx( mode, name )
	for temp in pairs(OWN_TEMPLET_LIST_INFO[ string.upper(mode) .. "_INFO" ] or {}) do
		APP_UTILS.tab_remove_ivalue(OWN_SEARCH_LIST_INFO[ temp ], name)
	end
	save_one_list( OWN_SEARCH_LIST_INFO, "SEARCH_LIST", false )
end
