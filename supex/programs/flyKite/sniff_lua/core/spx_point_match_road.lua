...
local x_redis_cmd_RL			= supex.mem_function_record( "redis_api_RL", redis_api.cmd, 20 )
local x_redis_cmd_LN			= supex.mem_function_record( "redis_api_LN", redis_api.cmd, 20 )
local x_redis_cmd_RI			= supex.mem_function_record( "redis_api_RI", redis_api.cmd, 20 )

module("spx_point_match_road", package.seeall)
...
local function point_match_road( lon, lat, dir, list )
	-->> parse data
	local array_lineID = {}
	local array_length = {}
	local array_angle1 = {}
	...
	for i=1, #valid_lineID do
		...
		if dist < G_MATCH_MAX_DIST and dist < rough_distance then
			rough_distance = dist
			index = i
		end
	end
	...
	--> 返回结果
	return {lineID = valid_lineID[index], roadID = valid_roadID[index], angle1 = valid_angle1[index], angle2 = valid_angle2[index]}
end

local function quick_location( lon, lat, dir, accountID )
	if not accountID then
		return false,nil
	end
	local old_roadID = supex.mem_classify_obtain("sysInternalRoadID", accountID)
	if not old_roadID then
		return false,nil
	end
	-- ERBR: export road by road
	local t1 = socket.gettime()
	--local ok,array_maybe_roadID = redis_api.cmd("roadRelation", accountID, "hmget", "ERBR", old_roadID)
	local ok,array_maybe_roadID = get_export_road_by_road( tonumber(old_roadID) )
	...
	for _,key in ipairs( valid_road_grid_key_array ) do
		local ok, kv_tab = x_redis_cmd_RL('mapRoadLine', accountID, 'hmget', key, array_maybe_roadID)
	...
	for idx,key in ipairs( valid_line_grid_key_array ) do
		...
		local ok, kv_tab = x_redis_cmd_LN('mapLineNode', accountID, 'hmget', key, field_array)
	...
	local result = point_match_road( lon, lat, dir, list )
	if not result then
		return false,nil
	end
	...
	if new_roadID ~= old_roadID then
		supex.mem_classify_record("sysInternalRoadID", accountID, new_roadID)
	end
	return true,result
end

local function whole_location( lon, lat, dir, accountID )
	-->> get 1/1000 grid keys
	...
	for _,key in ipairs( valid_grid_key_array ) do
		local ok, kv_tab = x_redis_cmd_LN('mapLineNode', accountID or "", 'hgetall', key)
...
function entry(direction, longitude, latitude, accountID)
	-->> check args
	...
	local ok, result = quick_location( lon, lat, dir, accountID )
	...
	if (not ok) or (not result) then
		...
		ok, result = whole_location( lon, lat, dir, accountID )
...
function detail_entry(direction, longitude, latitude, accountID, fields)
	local ok, result = entry(direction, longitude, latitude, accountID)
	if (ok and result) and ((not fields) or (type(fields) == "table" and #fields > 0)) then
		local key = result["roadID"] .. ":roadInfo"
		if not fields then
			local ok, kv_tab = x_redis_cmd_RI('mapRoadInfo', accountID or "", 'hgetall', key)
			--only.log('D', string.format("hgetall %s counts is %d", key, utils.get_sha_tab_count(kv_tab)))
			if ok and kv_tab then
			...
			local ok, kv_tab = x_redis_cmd_RI('mapRoadInfo', accountID or "", 'hmget', key, fields)
			if ok and kv_tab and (#kv_tab > 0) then
				for i,val in ipairs(kv_tab) do
					result[ fields[i] ] = val
				end
