...
local function app_call( top, msg, sch, way )
	only.log("D", '_________________________________START_________________________________________')
	APP_POOL["_FINAL_STAGE"] = top
	APP_POOL["_TASKER_SCHEME"] = sch
	--> get info
	...
	--> run call
	local path = nil--app_lua_get_path_data(sfd)
...
function app_call_all( top, msg, sch )
	app_call( top, msg, sch, false )
end

function app_call_one( top, msg, sch )
	app_call( top, msg, sch, true )
end
