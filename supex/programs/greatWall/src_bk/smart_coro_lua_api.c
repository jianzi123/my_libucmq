...
static int _vms_cntl( lua_State **L, int last, long S, struct smart_task_node *task )
{
...
{
	return smart_for_batch_vm( I, T, W, _vms_cntl );
}
...
static lua_State *_vms_new( void )
{
	...
	/*app init*/
	error = luaL_dofile(L, "lua/core/coro_start.lua");
	if (error) {
		fprintf(stderr,"%s\n", lua_tostring(L,-1));
		lua_pop(L,1);
		exit(EXIT_FAILURE);
...
static int _vms_init( lua_State **L, int last, long S, struct smart_task_node *task )
{
	if (*L != NULL){
...
{
	int error = smart_for_batch_vm( I, T, W, _vms_init );
	if (error) {
		exit(EXIT_FAILURE);
	}
	return error;
...
static int _vms_exit( lua_State **L, int last, long S, struct smart_task_node *task )
{
	int error = 0;
...
{
	int error = smart_for_batch_vm( I, T, W, _vms_exit );
...
static int _vms_rfsh( lua_State **L, int last, long S, struct smart_task_node *task )
{
...
{
	return smart_for_batch_vm( I, T, W, _vms_rfsh );
}



static int _vms_sync( lua_State **L, int last, long S, struct smart_task_node *task )
{
...
{
	return smart_for_batch_vm( I, T, W, _vms_sync );
}

/*=============================================================*/
static int _vms_gain( lua_State **L, int last, long S, struct smart_task_node *task )
{
...
{
	return smart_for_alone_vm( I, T, W, _vms_gain );
}



static int _vms_call( lua_State **L, int last, long S, struct smart_task_node *task )
{
...
{
	return smart_for_alone_vm( I, T, W, _vms_call );
}


static int _vms_exec( lua_State **L, int last, long S, struct smart_task_node *task )
{
...
{
	return smart_for_alone_vm( I, T, W, _vms_exec );
}
