...
int main(int argc, char** argv)
{
	...
	g_swift_cfg_list.func_info[ APPLY_FUNC_ORDER ].func = (LINE_TASK_CALLBACK)swift_vms_call;
	g_swift_cfg_list.func_info[ FETCH_FUNC_ORDER ].type = BIT8_TASK_TYPE_ALONE;
	g_swift_cfg_list.func_info[ FETCH_FUNC_ORDER ].func = (LINE_TASK_CALLBACK)swift_vms_gain;
	g_swift_cfg_list.func_info[ MERGE_FUNC_ORDER ].type = BIT8_TASK_TYPE_WHOLE;
	g_swift_cfg_list.func_info[ MERGE_FUNC_ORDER ].func = (LINE_TASK_CALLBACK)swift_vms_sync;
	g_swift_cfg_list.func_info[ CUSTOM_FUNC_ORDER ].type = BIT8_TASK_TYPE_ALONE;
	g_swift_cfg_list.func_info[ CUSTOM_FUNC_ORDER ].func = (LINE_TASK_CALLBACK)swift_vms_exec;
